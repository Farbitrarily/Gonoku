...
class Game_UI:
    ...
    def __init__(self, board, is_shown, **kwargs):
        ...
        if is_shown:
            ...
            # UI 资源
            self.__ui_chessboard = pygame.image.load(IMAGE_PATH+"chessboard2.jpg").convert()
            self.__ui_piece_black = pygame.image.load(IMAGE_PATH+"piece_white.png").convert_alpha()
            self.__ui_piece_white = pygame.image.load(IMAGE_PATH+"piece_black.png").convert_alpha()

            # 加载图标文件并创建Surface对象
            icon = pygame.image.load(IMAGE_PATH+"favicon.ico").convert_alpha()
